...
To turn them off by setting the C<filekeys(0)> attribute to false.

=head2 column_equations
...
Get/set this hash is a hash of hohs, containing the primary rows from the L</hoh> divided among the various 
...
Set/get the column name as a scalar that will be used to sort the L</hoh>.

   $hoh -> sort_column('D'); #sort using column D

See also the method L</sort_hoh>.

=head2 sort_type

Set/get the type of sort, where logical false, or C<0>, is a numerical comparison and logical true, or C<1>,
is a lexicographical string comparison. See L<http://perldoc.perl.org/perlop.html#Equality-Operators>. See 
also the method L</sort_hoh>.

=head2 unsorted

Set/get this boolean to logical true if you do not want the L</hoh> sorted upon saving.
...
Logical false, or C<0>, is ascending; and true is descending. See also the method L</sort_hoh>.

=head2 file

Set/get a full path/filename of the file that you will load into the L<Hoh> object.
...
Set/get a full path/filename of the file that you will save the L<Hoh> object as a flat file.
...
When boolean is true C<1>, then keys are generated by L<Hoh>, but when boolean is false C<0>
then the first column of the input file are consider the row keys for the L</hoh>. Use this
attribute in conjunction with L</filekeys>, which toggles whether the row keys will be saved
or not.

=head2 key_names

Get/set column name for the row keys column. This column name is only for file display purposes. 
It would be the first item in the header line, that is if the L</filekeys> attribute is set to true.

=head2 case_sensitive

Get/set this boolean that determines whether the data will be treated in a case-sensitive manner.
If set to false C<0> (default), then all file data is converted to upper case. When set to true,
...
Logical false C<0> (default) means they will not be saved. Logical true means they will.
...
Logical false C<0> (default) means it will be saved. Logical true means it will not be saved.
...
See the L</load> method since L</open_hoh> is an alias.
...
Save the L</hoh> hash data to a flat text file. The arguments are as follows: filename (scalar, required), print order (array
ref, not required), sort column (scalar, not required), boolean to decide if head is not printed (not required, C</0> means header is printed, where default is C<0>), delimiter (scalar, not required), and filekeys boolean (not required, for default C</0> the row 
...
These are all attributes in the L<Hoh> object so they may be set elsewhere in your implementation.
